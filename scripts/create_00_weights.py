...
if __name__ == '__main__':
    ...
    geocorr_df['GEOID00'] = (
        geocorr_df['county'] + geocorr_df['2000 Census Tract'].str.replace(
    ...
    total_pop_df = pd.DataFrame(
        geocorr_df.groupby('GEOID')['pop09'].sum()).reset_index()
    total_pop_df.rename(
        columns={
            'GEOID': 'GEOID00',
            'pop09': 'total_pop'
    ...
    crosswalk_join[
        'pop10'] = crosswalk_join['pop09'] * crosswalk_join['WEIGHT']
    ...
    geo_crosswalk = pd.DataFrame(
        crosswalk_join.groupby(
            ['geo_GEOID00',
             'geo_GEOID10'])['pop09', 'pop10'].sum()).reset_index()
