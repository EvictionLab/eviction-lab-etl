...
if __name__ == '__main__':
    ...
    if geo == 'counties':
        ...
        city_df = pd.read_csv(
            ...
            dtype={'GEOID00': 'object', 'GEOID10': 'object'}
        )
        city_dicts = city_df.to_dict(orient='records')
        city_crosswalk = {d['GEOID00']: {'GEOID': d['GEOID10'], 'name': d['NAME10']} for d in city_dicts}

        if 'name' in df.columns.values:
            df.loc[df['GEOID'].isin(city_crosswalk), 'name'] = df.loc[
                df['GEOID'].isin(city_crosswalk), 'GEOID'
            ].apply(lambda x: city_crosswalk[x]['name'])
        df.loc[df['GEOID'].isin(city_crosswalk), 'GEOID'] = df.loc[
            df['GEOID'].isin(city_crosswalk), 'GEOID'
        ].apply(lambda x: city_crosswalk[x]['GEOID'])
        ...
        for k, v in COUNTY_CROSSWALK.items():
            if 'parent-location' in df.columns.values:
                ...
                df.loc[df['GEOID'].str.startswith(k), 'GEOID'] = v['GEOID'] + df.loc[df['GEOID'].str.startswith(k), 'GEOID'].str.slice(5)
